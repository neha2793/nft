...
export default function SCVideo() {
  ...
  const [trFlag, SetTrFlag] = useState(false);
  ...
  function SCVideoList() {
    ...
    if(user){
      axios.get(process.env.NEXT_PUBLIC_BASE_URL+'/api/get-sc-video',{
        ...
        var dAfter = new Date();
        var nAfter = dAfter.getTime();
        var difference = nAfter - nBefore + 30;
      
        const interval = setInterval(() => {
          window.$('#categories_table').DataTable();
        }, 1000);
